...
export function ThemeProvider({
  ...
  const { getStyles, getImages, loading, error } = useProject(projectTitle);
  ...
  useEffect(() => {
    if (!loading && !error && styles) {
      ...
      if (images?.favicon && images.favicon.url) {
        ...
        link.href = process.env.NEXT_PUBLIC_API_URL + images.favicon.url;
      }
      ...
      if (theme.mode === "dark") {
        ...
        root.style.setProperty(
          "--bg-color",
          theme.backgroundColor === "#ffffff" ? "#1f2937" : theme.backgroundColor
        );
      ...
      document.body.style.color = theme.mode === "dark" ? theme.fontColorDark : theme.fontColor;
